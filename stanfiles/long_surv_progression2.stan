...
parameters {
  ...
  vector[X] beta;         // coefficients for each covariate; common between the submodels
...
model {
  ...
  for (n in 1:N) {
    progression[n] ~ poisson(exp(covars[n,]*beta)*base1[t[n]]*frailty[s[n]]);
    event[n] ~ poisson(exp(covars[n,]*beta)*base2[t[n]]*frailty[s[n]]);
  }
}
