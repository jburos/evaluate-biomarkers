...
## Multi-patient joint model

Next we're going to try running the multi-patient joint model on several patients of data.
...
jointdata <- list(
 ...
 , obs_t  = adata$t
 , obs_s = adata$id
 , obs_size = adata$rescaled_obs_size
 
 ## clinical event submodel
 , N = nrow(adata)
 , S = n_distinct(adata$id)
 , X = 1
 , T = max(adata$t)
 , s = adata$id
 , t = adata$t
 , event = adata$failure_or_progression
 , event_covars = adata %>% dplyr::select(rescaled_init_size)
)